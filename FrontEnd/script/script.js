...
fetchLogin = (event) => {
    ...
    }).then((response) => (response.status === 200) ? initSessionData(loginData) : console.log('Login error')).catch((error) => console.error('Error:', error))
...
firstRegistration = () => {
    if (document.querySelector('#inputPassword').value === document.querySelector('#inputPasswordCheck').value) {
        ...
        if (registerData.psw.length < 7 && registerData.name.length < 1 && registerData.email.lenght < 1) {
            ...
            }).then((response) => (response.status === 200) ? initSessionData(registerData) : console.log('Registrazione error')).catch((err) => console.log(err))
        }
...
fetchInformation = (event) => {
    ...
    }).then((response) => (response.status === 200) ? console.log('ok') : console.log('Registrazione error')).catch((err) => console.log(err))
